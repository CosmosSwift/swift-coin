...
extension NameServiceKeeper {
    ...
    func getWhois(request: Request, key: String) throws -> Whois? {
        ...
        guard let key = (NameServiceKeys.whoisPrefix + key).data(using: .utf8) else {
    ...
    func setWhois(request: Request, name: String, whois: Whois) throws {
        ...
        guard let key = (NameServiceKeys.whoisPrefix + name).data(using: .utf8) else {
            // TODO: throw a specific error
            throw Cosmos.Error.unknownRequest(reason: "invalid key")
        }
    ...
    func deleteWhois(request: Request, key: String) {
        let store = request.keyValueStore(key: storeKey)
        store.delete(key: (NameServiceKeys.whoisPrefix + key).data)
    }
    ...
    func listWhois(request: Request) -> Data {
        var whoisList: [Whois] = []
        let store = request.keyValueStore(key: storeKey)
        var iterator = store.prefixIterator(prefix: NameServiceKeys.whoisPrefix.data)
    ...
    func exists(request: Request, key: String) -> Bool {
        let store = request.keyValueStore(key: storeKey)
        return store.has(key: (NameServiceKeys.whoisPrefix + key).data)
    }
    ...
    func getNamesIterator(request: Request) -> Iterator {
        let store = request.keyValueStore(key: storeKey)
        return store.prefixIterator(prefix: NameServiceKeys.whoisPrefix.data)
    }
